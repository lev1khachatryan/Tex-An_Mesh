...
class RunModel(object):
    ...
    def build_test_model_ief(self):
        ...
        for i in np.arange(self.num_stage):
            ...
            verts, Js, _, weights = self.smpl(shapes, poses, get_skin=True, get_weights=True)
    ...
    def predict(self, images, get_theta=False, get_weights=False):
        """
        ...
        if get_theta:
            if get_weights:
                return results['joints'], results['verts'], results['cams'], results[
                    'joints3d'], results['theta'], results['weights']
            else:
                return results['joints'], results['verts'], results['cams'], results[
