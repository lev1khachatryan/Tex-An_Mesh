...
def simple_renderer(rn,
    ...
    numVerts = len(verts)
    numJoints = len(weights[0])
    maxBones = 4

    wWeights = np.zeros((numVerts, maxBones))
    wIndexs = np.zeros((numVerts, maxBones))
    wColors = np.zeros((numVerts, maxBones))
    ...
    while i < numVerts:
        ...
        count = 0
        j = 0
        while j < numJoints and count < maxBones:
            
            if weights[i][j] > 0:

                wWeights[i][count] = weights[i][j]
                index = (j+1)/numJoints
                wIndexs[i][count] = index                
                count += 1
    ...
    while i < numVerts:
        ...
        sortInd = np.argsort(wWeights[i])

        j = 0
        while j < maxBones:

            if indexs:
                wColors[i][j] = wIndexs[i][sortInd[maxBones-1-j]]
            else:
                wColors[i][j] = wWeights[i][sortInd[maxBones-1-j]]
...
def render_model(verts,
    ...
    imtmp = simple_renderer(rn, verts, weights, indexs, faces, color=color)
